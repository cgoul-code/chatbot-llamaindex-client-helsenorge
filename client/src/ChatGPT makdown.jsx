...
export default function ChatGPT() {
    ...
    const [interactions, setInteractions] = useState([]); // Array of objects with user question and assistant response
    const [formattedComponents, setFormattedComponents] = useState();
    ...
    const testAnswer = 
        ...
        "## overskrift **bold**\n" +
        "1. liste med *italic*"+
        "[Vurderer du å begynne på p - piller eller annen hormonell prevensjon ? — Ung.no](https://www.ung.no/prevensjon/2260_Praktiske_tips_om_hormonell_prevensjon.html)\n"+ 
        "1. ** Snakk med en voksen du stoler på**: Dette kan være foreldrene dine, en lærer, rådgiver, fastlege, psykisk helsearbeider eller helsesykepleier på skolen eller helsestasjon for ungdom(HFU).Det er mange voksne som ønsker å hjelpe deg hvis du gir dem en sjanse."
    ...
    }, [detailedResponse, empathiMode, youngAgeMode, chatGPTOnly, defaultSystemMessage, defaultSystemMessageChatGPTOnly], textInput, /*content*/);
    ...
    const handleStream = async (response, updateContentCallback) => {
        ...
        while (!done) {
            ...
            if (value) {
                ...
                chunk = chunk.replace(/\(\((.*?)\)\)/g, "<highlight>$1</highlight>");
...
// Custom rule to handle "==highlight==" syntax
    ...
    const handleSend = async (text = '') => {
        ...
        try {
            ...
            await handleStream(responseStream, updateContent);

            // const responseText = (await response.text()).trimStart();
            // // for testing markdowns
            // //const responseText = testAnswer;
            // //console.log(responseText);
            // // Parse the response and print the map


            // // Generate formatted components
            // const formattingMap = parseMarkdown(responseText);
            // //console.log('formattingMap:', formattingMap);
            // const components = generateFormattedComponents(formattingMap);
            // setFormattedComponents(components);

            // // Update the latest interaction with the assistant's response
            // // Add user message to interactions
            // console.log("FormatedComponents:", formattedComponents)
            // setInteractions((prevInteractions) => [
            //     ...prevInteractions,
            //     { userMessage: userMsg, assistantResponse: components } // Placeholder for assistant response
            // ]);

            // setInteractions((prevInteractions) => {
            //     const updatedInteractions = [...prevInteractions];
            //     updatedInteractions[updatedInteractions.length - 1].assistantResponse = responseText;
            ...
            <View
                ...
                <View style={[styles.col4, {
                    ...
                    <select style={styles.selectContainer}
                        ...
                        <option value="ungno5000spm">Alle temaene - 5000 spørsmål&svar</option>
                        <option value="psassa">Hva er innafor - Temasider</option>
                        <option value="hvaerinnafor">Hva er innafor - tekstene</option>
                    </select>
                </View>
                <View style={[styles.col1, { flex: 2 }]}></View>
            </View>
...
const styles = StyleSheet.create({
    ...
    /*     paragraph: {
            fontSize: 12,
            lineHeight: 14,
            marginBottom: 5, // Adds space between list items
            marginTop: 5,
            color: 'red',
        },
        list: {
            paddingLeft: 8, // Adds some indentation for the list
            color:'yellow',
        },
        listItemContainer: {
            flexDirection: 'row', // Aligns bullet and text horizontally
            alignItems: 'flex-start',
            marginBottom: 0, // Adds space between list items
            fontSize: 12,
            color: 'blue'
        ...
        bullet: {
            fontSize: 12,
            marginRight: 4, // Space between bullet and
            paddingTop: 9, 
            marginLeft: 4,
            //color: 'blue',
        ...
        listItem: {
            fontSize: 12, // Set font size for list items
            paddingTop: 9,
            lineHeight: 14,
            marginLeft: 4,
            flex: 1, // Ensures the text wraps within the container
            color: 'cyan',
